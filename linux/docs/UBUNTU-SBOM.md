...
Target-neutral selectable metadata is maintained in
[`victuals/definitions/`](../../victuals/definitions/); this SBOM remains the
implementation inventory for the Ubuntu adapter.

## Bootstrap phase (`hydrate-ubuntu.sh`)

On a fresh Ubuntu host, start the one terminal-only Victuals workflow directly because
Go Task does not exist yet:

```bash
bash linux/scripts/hydrate-ubuntu.sh
```

To exercise the real Gum questionnaire and final confirmation while testing
only the Snap-removal path, use the focused mode below. It installs bootstrap
prerequisites, prompts for the full selection set, then requires the literal
`REMOVE-SNAP` confirmation before removing Snap. No application, desktop,
power, GPU, or service hydration runs in this mode:

```bash
bash linux/scripts/hydrate-ubuntu.sh snap-only
```
...
| `curl` | Transfer client used by the Victuals workflow for repositories and vendor installers. | `curl`. | APT, pre-hydration bootstrap |
...
| Desktop | GNOME Tweaks | GUI | APT | Advanced GNOME preferences. | GNOME policy is applied separately by the Victuals workflow. |
...
| Accessibility | Speech Note | GUI | Alternative | Mature offline dictation application from Flathub. | Flatpak only; model and microphone setup are first-run. |
| Accessibility | Vocalinux | GUI | Low-resource | VOSK-based offline dictation AppImage. | User wrapper/desktop entry with AppImage extraction mode; model/microphone setup is first-run. |
| Accessibility | Voquill | GUI | Alternative | Cross-platform dictation with account and trial flows. | User wrapper/desktop entry with AppImage extraction mode; account/model setup is first-run. |
| Accessibility | Voxtype | GUI | Recommended | Omarchy-associated local dictation application. | Latest official GitHub release `.deb` installed via APT; dependencies are defined in the package; no apps or configs depend on it; first-run setup is required. |
